...
| Usage snapshot | `GET /usage`（過進站閘門）回 token pool 每顆的帳號身分（email / plan / rate_limit_tier）、5h 與 7d utilization / resets_at、狀態（active / standby / benched / unusable，active 為 acquire 會選中的第一顆），加 active 帳號的 `models[]`。快取 60s（`USAGE_CACHE_TTL_SECONDS`，對齊前端輪詢）；過期 token 會先 refresh，單一帳號失敗不影響整體 snapshot；抓取失敗（如 OAuth endpoint 429）時退回該帳號上次成功資料並標 `stale: true` | `src/claude_code_proxy/service.py`（`usage_snapshot`）+ `src/claude_code_proxy/app.py` |
